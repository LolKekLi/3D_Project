...
namespace ToonyColorsPro
{
	namespace ShaderGenerator
	{
		public class MaterialInspector_Hybrid : ShaderGUI
		{
							...
							if (insideProperties)
							{
								...
								if (line.StartsWith(kGuiCommandPrefix))
								{
									...
									else if (command == kGC_HelpBox)
									{
										...
										var cmd = fullCommand.Substring(fullCommand.LastIndexOf(kGC_HelpBox) + kGC_HelpBox.Length + 1).Split(new string[] { "::" }, StringSplitOptions.RemoveEmptyEntries);
										if (cmd.Length == 1)
										{
											message = cmd[0];
										...
										else if (cmd.Length == 2)
										{
											try
											{
												var msgType = (MessageType)Enum.Parse(typeof(MessageType), cmd[0], true);
			...
			void IterateMaterialsByIndex(Action<Material, int> action)
			{
				int i = 0;
				foreach (var target in this._materialEditor.targets)
				{
